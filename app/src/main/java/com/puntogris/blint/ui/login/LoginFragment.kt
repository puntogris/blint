...
class LoginFragment : BaseFragment<FragmentLoginBinding>(R.layout.fragment_login) {
                ...
                try {
                        ...
                        lifecycleScope.launchWhenCreated {
                            viewModel.logInUserToFirestore(credentialToken).collect { result ->
                                when (result) {
                                    ...
                                    is AuthResult.Error -> {
                                        createShortSnackBar(result.exception)
                                    }
                                }
                            }
                        }
                    ...
                    when (e.statusCode) {
                        ...
                        CommonStatusCodes.NETWORK_ERROR ->
                            createShortSnackBar("Se encontro un problema con la conexion. Revisa tu red y intenta nuevamente.")
                        else ->
                            createShortSnackBar("Estamos teniendo dificultades tecnicas. Intenta nuevamente en un rato.")
                    }
                }
            }
        }
    ...
    fun onLoginButtonClicked(){
        ...
        else createShortSnackBar("Debido a multiples intentos de ingreso consecutivos, se deshabilito el ingreso a la app momentaneamente.")
    }
